...
def load_email_recipients(csv_path: str) -> Dict[str, str]:
    ...
    try:
        ...
        email_dict = dict(zip(emails_df['Region'], emails_df['Email']))
        return email_dict
...
def email_reports(
    ...
    try:
        ...
        for i, region in enumerate(regions):
            ...
            mail.To = recipients[region]
            
            # Email body
            mail.HTMLBody = f"""
            <p>Hello,</p>
            <p>Attached is the Automated Traffic Signal Performance Measures (ATSPM) report for {region} dated {today}.</p>
            <p>This report includes analysis of phase termination patterns and detector health for signals in your region.</p>
            ...
            if report_in_memory:
                ...
                temp_path = f"temp_report_{region.replace(' ', '_')}.pdf"
